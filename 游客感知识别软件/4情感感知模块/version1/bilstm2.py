...
for e in range(epochs):
    ...
    print(f'Epoch: {e+1:02}, Train Loss: {epoch_loss_train:.3f}, Train Acc: {sum(batch_acc_train) / len(batch_acc_train) * 100:.2f}%')
...
print(f'test Loss: {epoch_loss_test:.3f}, Test Acc: {sum(batch_acc_test) / len(batch_acc_test) * 100:.2f}%')

filename = "checkpoints/{}_{}_bs{}_hd{}_acc{:.03f}.pth".format(
                    'bilstm', 'adam', '50', '32', sum(batch_acc_test) / len(batch_acc_test) * 100
                )
